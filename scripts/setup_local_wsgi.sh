...
# Check arguments
[ -z "$env" ] && echo Usage: $0 '<environment>' 2>&1 && exit 1
...
[ -z "$DJANGO_PROJECT" ] && \
    echo "Variable DJANGO_PROJECT not set or empty" 2>&1 && exit 1
...
[ ! -f "$wsgi_target" ] && echo $wsgi_target not found - exiting 2>&1 && exit 1

rm -f $wsgi_local
ln -s $env.wsgi $wsgi_local
[ ! $? -eq 0 ] && echo Error creating symlink $wsgi_local 2>&1 && exit 1
echo Link $local.wsgi to $wsgi_target created. 2>&1
...
[ ! -f "$settings_target" ] && echo $settings_target not found - exiting 2>&1 && exit 1
